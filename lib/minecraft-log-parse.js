...
module.exports = class {
  ...
  parseLogLine(line) {
    ...
    if (matches) {
      ...
      // Lag
        ...
        //Can't keep up! Did the system time change, or is the server overloaded? Running 2040ms behind, skipping 40
        // tick(s)
      ...
      // Login IP and Coords
        ...
        // roamer7038[/202.180.86.136:34280] logged in with entity id 1437768 at ([world]602.2744076595088, 64.0,
        // -39.02937949785859)
      ...
      // Death
      else if (type === 'server thread/info' && this.regDeathLog(text)) {
        let breakdown = this.regDeathLog(text, 'true');

        return {
          'time': time,
          'type': 'death',
          'user': breakdown.origin[1],
          'text': text,
   ...
   * 一致しない場合は null を返す
   * 引数 isTrans が true の場合は日本語訳も返す
   */

  regDeathLog(text, isTrans) {
    ...
    if (isTrans) {
      trans = death.trans;
      trans = trans.replace('$1', matched[1]);
      if (this.isset(matched[2]))
        trans = trans.replace('$2', matched[2]);
      if (this.isset(matched[3]))
        trans = trans.replace('$3', matched[3]);
    }
